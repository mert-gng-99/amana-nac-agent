...
1. Friday payday remittance, the first scenario on the left. Watch the agent pick
...
2. Run all six transfers runs all 6 scenarios back to back and shows the
...
and all 6 scenarios reaching the outcome they claim.
...
```bash
docker build -t amana . && docker run -p 8000:8000 amana
```
