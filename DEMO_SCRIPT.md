...
Click **Friday payday remittance**.
...
Click **Run all six transfers**.

> "All 6 scenarios. Every one reached the outcome it claims, and the
> demo asserts the same thing the test suite does."

Point at the cost line.

> "35 units spent. Calling every available check on every case would
> have cost 72. That is 51.4% saved, and look at the spread:
> *Friday payday remittance* took 1 call, *SIM changed six hours ago* earned 5. An
...
> "Amana. A cost-aware fraud agent for cross-border transfers. 4 CAMARA APIs on Nokia Network as Code,
...
Run the two SIM swap scenarios back to back. Both return the same CAMARA answer; only the timestamp differs, and the decisions differ with it.
...
- Scenarios worth using: Friday payday remittance, First transfer to a new receiver, SIM changed six hours ago
