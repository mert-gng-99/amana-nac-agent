# Amana - Prototype Phase submission
...
**YOU.** Pick your Idea Phase submission for Amana from the dropdown.
...
```
4. Secure Fintech, Payments & Anti-Fraud Innovation
```
...
1. The dashboard after Run all six transfers, showing the batch table and the cost comparison.
...
```
Amana Pitch Deck.pptx
```

It sits in the parent folder of this bundle. Update the closing slides with
the prototype numbers before uploading: 6 scenarios, all passing,
4 CAMARA APIs, 51.4% cheaper than calling every check.
...
```
https://github.com/mert-gng-99/amana-nac-agent
```
...
| --- | --- | --- |
| Parent Submission | Pick Amana from the dropdown | seconds |
...
| --- | --- |
| At least one CAMARA API on Nokia NaC | 4 API families: `number-verification`, `sim-swap`, `device-swap`, `device-status` |
...
| Solves a real problem | Secure Fintech, Payments & Anti-Fraud Innovation. Stated limits are in README.md, so this is not pitched as doing more than it does. |
| Theme alignment | Theme 4 |
| Multiple CAMARA APIs (good-to-have) | 4 families orchestrated per case |
| Intelligent orchestration (good-to-have) | 1 to 5 calls depending on the case; 51.4% cheaper than calling everything |
| Production-minded (good-to-have) | Consent in the transport path, evidence ledger, Docker and Render config, 6 scenario tests plus transport, agent and API suites |
